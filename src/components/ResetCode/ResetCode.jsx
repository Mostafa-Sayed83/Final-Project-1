...
export default function ResetCode() {
    ...
    <form onSubmit={formik.handleSubmit} className='md:px-16 relative'>
        <div className="mb-2 relative">
            <label htmlFor="resetCode" className="block mb-2 text-sm font-normal text-gray-600 text-left">Code :</label>
            <input 
            type="text"
            name="resetCode"
            ...
            {formik.errors.resetCode && formik.touched.resetCode?<div className="p-4 my-4 text-sm border border-red-600 text-red-600 rounded-lg bg-red-50 text-left" role="alert">
            <span className="font-medium">{formik.errors.resetCode}</span></div> : null}
        </div>
        ...
        <button type="submit" disabled={isLoading} className="text-white md:mx-16 md:mt-8 md:px-10 bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg px-5 py-2.5 text-center left-0  absolute">
            {isLoading ? <i class="fa-solid fa-spinner fa-spin"></i> : "Verify"}
        </button>
        </div>
    </form> 
    </>
